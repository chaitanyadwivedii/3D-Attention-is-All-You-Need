...
from slowfast.models import head_helper, resnet_helper, stem_helper, cnn_bridge
...
#         print(np.shape(x[0]), np.shape(x[1]))
        x2 = self.cnn_bridge(x, query)
#         print("-"*100)
#         print("Head ")
#         print(x2.size(), self.training, x2)
#         print("-"*100)
#         x = self.head(x)
...
class ResNetModel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.s4(x)
        print("_"*25)
        print("Shape of x4 output", type(x))
        x = self.s5(x)
        x = self.head(x)
        return x
